...
// DB 테이블 이름 마이그레이션 (crawlers → scrapers)
for (const [oldName, newName] of [['crawlers', 'scrapers'], ['crawl_results', 'scrape_results']]) {
  if (db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`).get(oldName)) {
    db.exec(`ALTER TABLE ${oldName} RENAME TO ${newName}`);
  }
}
for (const [table, oldCol, newCol] of [
  ['scrape_results', 'crawler_id',   'scraper_id'],
  ['heal_proposals', 'crawler_id',   'scraper_id'],
  ['heal_proposals', 'crawler_name', 'scraper_name'],
]) {
  const exists = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`).get(table);
  if (exists) {
    const cols = db.prepare(`PRAGMA table_info(${table})`).all();
    if (cols.find(c => c.name === oldCol)) {
...
const stmts = {
  list:   db.prepare('SELECT * FROM scrapers ORDER BY created_at DESC'),
  get:    db.prepare('SELECT * FROM scrapers WHERE id = ?'),
  insert: db.prepare(`
    INSERT INTO scrapers (id, name, url, css_selector, user_intent, threshold, schedule, channels, domain, org, owner, status)
  ...
  updateSelector: db.prepare(`
    UPDATE scrapers SET css_selector = @css_selector, user_intent = @user_intent,
      status = 'pending', score = 0, last_value = '—'
    WHERE id = @id
  `),
  _deleteResults: db.prepare('DELETE FROM scrape_results WHERE scraper_id = ?'),
  _deleteScraper:  db.prepare('DELETE FROM scrapers WHERE id = ?'),
};

const resultStmts = {
  insert: db.prepare(`
    INSERT INTO scrape_results (scraper_id, status, value, score, duration_ms, note)
    VALUES (@scraper_id, @status, @value, @score, @duration_ms, @note)
  `),
  list:        db.prepare('SELECT * FROM scrape_results WHERE scraper_id = ? ORDER BY run_at DESC LIMIT 50'),
  sparkScores: db.prepare('SELECT score FROM scrape_results WHERE scraper_id = ? ORDER BY run_at ASC'),
  updateLastScore: db.prepare(`
    UPDATE scrape_results SET score = @score
    WHERE id = (SELECT id FROM scrape_results WHERE scraper_id = @scraper_id ORDER BY run_at DESC LIMIT 1)
  `),
};

const statsStmts = {
  activeFeeds:  db.prepare("SELECT COUNT(*) as n FROM scrapers WHERE status != 'paused'"),
  ...
  `),
  totalHealed:  db.prepare("SELECT COALESCE(SUM(healed_count), 0) as n FROM scrapers"),
  pendingCount: db.prepare("SELECT COUNT(*) as n FROM heal_proposals WHERE status='pending'"),
  durations7d:  db.prepare(
    "SELECT duration_ms FROM scrape_results WHERE run_at >= datetime('now', '-7 days', 'localtime') ORDER BY duration_ms"
  ),
};

const proposalStmts = {
  insert: db.prepare(`
    INSERT INTO heal_proposals (scraper_id, scraper_name, old_selector, proposed_selector, extracted_text, confidence, reasoning)
    VALUES (@scraper_id, @scraper_name, @old_selector, @proposed_selector, @extracted_text, @confidence, @reasoning)
  `),
  ...
  _deleteByScraper: db.prepare(`DELETE FROM heal_proposals WHERE scraper_id = ?`),
};

function dbRowToScraper(row) {
  if (!row) return null;
  return {
    id:           row.id,
    name:         row.name,
...
module.exports = {
  scrapers: {
    list:   ()     => stmts.list.all().map(dbRowToScraper),
    get:    (id)   => dbRowToScraper(stmts.get.get(id)),
    insert: (data) => { stmts.insert.run(data); return module.exports.scrapers.get(data.id); },
    update: (data) => { stmts.update.run(data); return module.exports.scrapers.get(data.id); },
    updateSelector: (id, css_selector, user_intent) => {
      stmts.updateSelector.run({ id, css_selector, user_intent });
      return module.exports.scrapers.get(id);
    },
    delete: db.transaction((id) => {
      proposalStmts._deleteByScraper.run(id);
      stmts._deleteResults.run(id);
      stmts._deleteScraper.run(id);
    }),
  },
  results: {
    ...
    updateLastScore: (scraper_id, score) => resultStmts.updateLastScore.run({ scraper_id, score }),
  },
  stats: {
    summary: () => {
